
import React from 'react'
import { connect } from 'react-redux'
import {
    Button,
    Form,
    FormGroup,
    FormFeedback,
    Label,
    Input,
} from 'reactstrap'

import { editContact, clearEditingContactError } from '../../actions/contacts'
import { msgFromError } from '../../util'


class EditContactForm extends React.Component {
    constructor (props) {
        super(props)

        this.state = {
            type: this.props.contact.get('type', 'Regulatory'),
            title: this.props.contact.get('title', ''),
            contact: this.props.contact.get('contact', ''),
            phone: this.props.contact.get('phone', ''),
            cell: this.props.contact.get('cell', ''),
            fax: this.props.contact.get('fax', ''),
            email: this.props.contact.get('email', ''),
            address: this.props.contact.get('address', ''),
            city: this.props.contact.get('city', ''),
            state: this.props.contact.get('state', ''),
            zip: this.props.contact.get('zip', ''),
            notes: this.props.contact.get('notes', ''),
        }
    }

    componentWillMount () {
        if (this.props.editingContactError) {
            this.props.clearEditingContactError()
        }
    }

    onChange (e) {
        if (this.props.editingContactError) {
            this.props.clearEditingContactError()
        }

        this.setState({
            [e.target.name]: e.target.value,
        })
    }

    onSubmit (e) {
        e.preventDefault()
        this.props.editContact(this.props.contact.get('id'), {
            type: this.state.type,
            title: this.state.title,
            contact: this.state.contact,
            phone: this.state.phone,
            cell: this.state.cell,
            fax: this.state.fax,
            email: this.state.email,
            address: this.state.address,
            city: this.state.city,
            state: this.state.state,
            zip: this.state.zip,
            notes: this.state.notes,
            site_id: this.props.site.get('id'),
        })
        .then(this.props.onSuccess)
    }

    render () {
        const error = this.props.editingContactError
        const generalError = error && error.msg ? error.msg : null
        const errors = error && error.key ? {
            [error.key]: msgFromError(error),
        } : {}

        return (
            <Form onSubmit={e => this.onSubmit(e)}>
                <FormGroup color={errors.type ? 'danger' : ''}>
                    <Label for="type">Type</Label>
                    <Input
                        state={errors.type ? 'danger' : ''}
                        type="select"
                        name="type"
                        id="type"
                        value={this.state.type}
                        onChange={e => this.onChange(e)}
                    >
                        <option>Regulatory</option>
                        <option>Account Holder/Invoicing</option>
                        <option>Consultant/Engineer</option>
                        <option>Laboratory</option>
                    </Input>
                    <FormFeedback>{errors.type}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.title ? 'danger' : ''}>
                    <Label for="title">Company Name</Label>
                    <Input
                        state={errors.title ? 'danger' : ''}
                        name="title"
                        id="title"
                        value={this.state.title}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.title}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.contact ? 'danger' : ''}>
                    <Label for="contact">Contact</Label>
                    <Input
                        state={errors.contact ? 'danger' : ''}
                        name="contact"
                        id="contact"
                        value={this.state.contact}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.contact}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.phone ? 'danger' : ''}>
                    <Label for="phone">Office Phone #</Label>
                    <Input
                        state={errors.phone ? 'danger' : ''}
                        name="phone"
                        id="phone"
                        value={this.state.phone}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.phone}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.cell ? 'danger' : ''}>
                    <Label for="cell">Mobile Phone #</Label>
                    <Input
                        state={errors.cell ? 'danger' : ''}
                        name="cell"
                        id="cell"
                        value={this.state.cell}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.cell}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.fax ? 'danger' : ''}>
                    <Label for="fax">Fax</Label>
                    <Input
                        state={errors.fax ? 'danger' : ''}
                        name="fax"
                        id="fax"
                        value={this.state.fax}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.fax}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.email ? 'danger' : ''}>
                    <Label for="email">Email</Label>
                    <Input
                        state={errors.email ? 'danger' : ''}
                        name="email"
                        id="email"
                        value={this.state.email}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.email}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.address ? 'danger' : ''}>
                    <Label for="address">Address</Label>
                    <Input
                        state={errors.address ? 'danger' : ''}
                        name="address"
                        id="address"
                        value={this.state.address}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.address}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.city ? 'danger' : ''}>
                    <Label for="city">City</Label>
                    <Input
                        state={errors.city ? 'danger' : ''}
                        name="city"
                        id="city"
                        value={this.state.city}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.city}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.state ? 'danger' : ''}>
                    <Label for="state">State</Label>
                    <Input
                        state={errors.state ? 'danger' : ''}
                        name="state"
                        id="state"
                        value={this.state.state}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.state}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.zip ? 'danger' : ''}>
                    <Label for="zip">Zip</Label>
                    <Input
                        state={errors.zip ? 'danger' : ''}
                        name="zip"
                        id="zip"
                        value={this.state.zip}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.zip}</FormFeedback>
                </FormGroup>
                <FormGroup color={errors.notes ? 'danger' : ''}>
                    <Label for="notes">Notes</Label>
                    <Input
                        state={errors.notes ? 'danger' : ''}
                        type="textarea"
                        name="notes"
                        id="notes"
                        value={this.state.notes}
                        onChange={e => this.onChange(e)}
                    />
                    <FormFeedback>{errors.notes}</FormFeedback>
                </FormGroup>
                <Button
                    role="button"
                    color="primary"
                    disabled={this.props.editingContact}
                >Save</Button>
            </Form>
        )
    }
}

const mapStateToProps = store => ({
    editingContactError: store.get('editingContactError'),
    editingContact: store.get('editingContact'),
})

const mapDispatchToProps = dispatch => ({
    editContact: (id, contact) => dispatch(editContact(id, contact)),
    clearEditingContactError: () => dispatch(clearEditingContactError()),
})

export default connect(mapStateToProps, mapDispatchToProps)(EditContactForm)
